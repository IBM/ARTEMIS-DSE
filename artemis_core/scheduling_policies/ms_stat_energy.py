...
class SchedulingPolicy(BaseSchedulingPolicy):
    ...
    def assign_rank_and_type(self, t, wcet_slack, bcet_slack, actual_task_slack, actual_dag_slack):
        worst_energy, worst_energy_pe, best_energy, best_energy_pe = t.worst_energy,t.worst_energy_pe, t.best_energy, t.best_energy_pe
        #OLD
        ...
        # else:
        #     if wcet_slack >= 0:
        #         # If task can execute on any processor and meet deadline, priortise the one which has lower energy first
        #         t.rank = -int(worst_energy)
        #         t.rank_type = 4
        #     elif bcet_slack >= 0:
        #         slack = bcet_slack
        #         t.rank = slack
    ...
    def calculate_slacks(self, kernel, clock_time):
        ...
        assert kernel.min_time is not None
        assert kernel.max_time is not None
    ...
    def assign_kernel(self, clock_time, kernel, window, scheduled_kernels, yet_to_schedule_kernels):
        ...
        for kernel in window:
            # print(f"---> [{clock_time}] IN assign_kernel, trying to schedule {kernel.task_name}")
            # s0 = time.time()
            ...
            if not kernel.get_task().is_task_dummy():
                ...
                # IMPORTANT: we will select the block that minimizes this metric under the constraint that the block will make the kernel meet its deadline
                # the metric is (mean service time + waiting time) * (energy of kernel on block)
                target_block_min_metric = [] # this is a tuple of metric and actual service time
                ...
                for block in self.blocks:
                    ...
                    if block.instance_type in self.task_to_mappable_pe_map[kernel.task_name]:
                        if block.busy:
                            ...
                            # if kernel.get_task_name() == "VitPre_x1_3_5":
                            #     print(f"                        task_on_block_est_lat of {block.kernel_running.task_name} on {block.instance_name} is {task_on_block_est_lat}")
                            ...
                            for stask in window:
                                ...
                                if block.instance_name == stask.possible_block_instance_name:
                                    ...
                                    # if kernel.get_task_name() == "VitPre_x1_3_5":
                                    #     print(f"                    stask_est_lat of {stask.task_name} on {block.instance_name} is {stask_est_lat}")
                        ...
                        # Get energy of the prospective block
                        isolation_energy = kernel.get_energy_if_krnel_run_in_isolation(block)
                        min_metric = isolation_energy * actual_service_time
                        ...
                        target_block_min_metric.append((min_metric, actual_service_time))
                    else: # for incompatible blocks
                        target_block_time.append(float("inf"))
                        target_block_min_metric.append((float("inf"), float("inf")))
                    ...
                    #     continue
                if kernel.rank_type == 1:
                    copy_target_block_min_metric = copy.deepcopy(target_block_min_metric)

                    # Order the min_metric list first by the metric and then by actual service time, both in ascending order
                    copy_target_block_min_metric.sort(reverse=False)
                    assert copy_target_block_min_metric[0] == min(target_block_min_metric)

                    # Find the block that has the least min_metric and meets the deadline
                    for min_metric in copy_target_block_min_metric:
                        block_idx = target_block_min_metric.index(min_metric)
                        target_time = target_block_time[block_idx]
                        # print(f"target_block_time[{self.blocks[block_idx].instance_name}] = {target_time}, kernel.deadline = {kernel.deadline}")
                        if target_time <= kernel.deadline: #Deadline less than actual service time
                            # print(f"found")
                            block_to_assign = self.blocks[block_idx]
                            break
                    
                    # Even if none of the blocks meet the deadline assign the block with the fastest finish time 
                    # ---> This case might still happen with rank_type 1 if remaining_time causes us to miss the deadline
            ...
            # s1 = time.time()
            # print(f"task_to_assign = {kernel.task_name} block_to_assign = {block_to_assign.instance_name}, busy = {block_to_assign.busy}")
