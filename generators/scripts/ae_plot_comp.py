    ...
    for metric in metric_all:
        ...
        for dline in deadlines:
            ...
            for iat in dag_iat_all:
                logger.warning(f"DAG IAT: {iat}")
                if IAT_MIN != None and iat < IAT_MIN:
                    continue
                x_val = int(round(1/iat, 0)) # DAG arrival rate in Hz
                for ncv_nrad_nvit in ncv_nrad_nvit_all:
                    nc, nr, nv = ncv_nrad_nvit
                    logger.warning(f"\tN_CV, N_rad, N_vit: {ncv_nrad_nvit}")
                    for k, frame in enumerate(frame_all):
                        ...
                        if not len(csv_file_full) == 1: # , f"expected only 1 match for {csv_file}, but found {csv_file_full}"
                            ...
                            logger.info(f"Reading file: {csv_file_full}")
            ...
            for i, ncv_nrad_nvit in enumerate(ncv_nrad_nvit_all):
                ...
                for k, frame in enumerate(frame_all):
                    ...
                    if metric in ["lat_wall_time", "lat_pow", "lat_area"]:
                        ...
                        x_ind = np.arange(num_xpoints+1)
                    if metric in ["lat_wall_time", "lat_pow", "lat_area"]:
                        logger.info("metric:{}, dline:{}, ncv_nrad_nvit:{}, frame:{}, plot_y_twin[-1]:{}, gm:{}".format(metric, dline, ncv_nrad_nvit, frame, plot_y_twin[-1], gm))
                    else:
                        logger.info("metric:{}, dline:{}, ncv_nrad_nvit:{}, frame:{}, gm:{}".format(metric, dline, ncv_nrad_nvit, frame, gm))
            ...
            axs[0].legend(handles, labels, loc='upper center', ncol=3, fontsize=6)
...
if __name__ == "__main__":
    ...
    pol_sel = {
        (1,2,1.0): "ms_dyn_energy", # in paper, ART_DSE
    ...
    if reduced_exp_set:
        dag_iat_all = dag_iat_all[::2]
    ncv_nrad_nvit_all = [
        (1, 2, 1.0), 
        (1, 4, 4.0)
    ]
    sched_pol = "ms_dyn_energy"
    plot_one(log_dir["real"], "real", 'h', SOC_TYPE="farsi",     TOP_TYPE="ct", sched_pol=sched_pol, dag_iat_all=dag_iat_all, ncv_nrad_nvit_all=ncv_nrad_nvit_all, color={"bar": ['#d7191c', '#fdae61', '#2c7bb6'], "line": ['#d7191c', '#fdae61', '#2c7bb6']})
    plot_one(log_dir["real"], "real", 'h', SOC_TYPE="farsi",     TOP_TYPE="ut", sched_pol=sched_pol, dag_iat_all=dag_iat_all, ncv_nrad_nvit_all=ncv_nrad_nvit_all, color={"bar": ['#d7191c', '#fdae61', '#2c7bb6'], "line": ['#d7191c', '#fdae61', '#2c7bb6']})
    plot_one(log_dir["real"], "real", 'h', SOC_TYPE="fixed_het", TOP_TYPE="ut", sched_pol=sched_pol, dag_iat_all=dag_iat_all, ncv_nrad_nvit_all=ncv_nrad_nvit_all, color = {"bar": ["#d7191c", "#2c7bb6"], "line": ["#d7191c", "#2c7bb6"]})